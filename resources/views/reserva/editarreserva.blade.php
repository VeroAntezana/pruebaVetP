...
<h1 class="text-3xl text-center font-bold">Editar Reserva {{$reserva['id']}}</h1>

<form class="mt-4" method="POST" action="{{route('admin.updatereserva', $reserva->id)}}">
@csrf
...
<label for="">Mascota</label>
    ...
    @enderror
            ...
            <div>
                ...
                <input id="fecha" name="fecha" type="date" class="border border-gray-200 rounded-md bg-gray-200 w-full text-lg placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{$reserva['fecha']}}">
                @error('fecha')
     <p class="border border-red-500 rounded-md bg-red-100 w-full text-red-600 p-2 my-2">{{ $message }}</p>
    @enderror
            </div>
